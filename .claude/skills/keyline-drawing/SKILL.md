...
- `resize.mjs` 的 plate 重拟合只在「plate 就是整枚字形的轮廓」时对。plate 只覆盖一部分的字形（sun-medium 是圆盘，光芒在外面）会被拟合成整个油墨盒：r5.5 变 r11.11，直接顶穿 padding。这类字形缩完要自己按缩后的中心线重算 plate（外扩 1），别信工具的输出。同一次拟合还会把多子路径的 plate（quote 的两个逗点）外缘对上、内缘各胖出一个笔宽差，SPACING 报 0.02 的重叠；按各自外缘把每条子路径收回到对应描边的油墨宽度即可。
- 斜向自由端按轴向的 +1 延长，在 padding 下限 0.99 的年代是 error（2026-09-05 上游收紧的，此前 sharp 是 0.586）。判据：butt 的角在 45° 上比中心线端点多探 1.414，圆头只多 1。修法是沿自身方向回收到「butt 角离画布边 1.0858」，那正是 §2 的 +0.293 在端点离边 2 单位时落的位置。近乎垂直的自由端（scribble 左端）回收到齐平圆角端点即可，再退就吃到笔画本身了。
