// Scan the keyline glyphs do-tv uses and report how big each one paints, judged the way lint judges them.
//   node .claude/skills/keyline-drawing/scripts/scan-dotv-sizes.mjs [do-tv-web-dir] [out.html]
// Reads the semantic layer (icons.ts + icons-editing.ts), measures icons/stroke/<name>.svg (stroke paths reach 1,
// filled marks reach 0), classifies each glyph (horizontal / vertical / square / circle / bare) and flags the ones
// that paint smaller than their class asks: horizontal → 22 wide, vertical → 22 tall, square → 20, circle → 22,
// bare → max side 20…22.5 (below 19.5 is what the eye reads as "small" in a menu). Writes an HTML sheet if asked.
import fs from "node:fs";
import path from "node:path";
import { flatten } from "./svgflat.mjs";
const here = path.dirname(new URL(import.meta.url).pathname);
const R = path.resolve(here, "../../../..");
const DOTV = process.argv[2] || path.resolve(R, "../do-tv/packages/web");
...
for (const file of ["icons.ts", "icons-editing.ts"]) {
