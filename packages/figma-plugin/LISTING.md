...
```
678 icons in three styles, cut rounded or sharp.
```
...
```
Search 678 icons and drop one on the canvas. No library to publish, no file to duplicate, no plan requirement.

Three styles

* Stroke: 678 icons, 2px, round caps and joins
* Duotone: 556 icons, a 40% plate under the line
* Fill: 506 icons, solid where the glyph has a region to fill
...
Search knows more than the file names. 558 icons carry curated words, so "south" finds arrow-down, "hamburger" finds menu and "trash" finds bin. Paste a component name straight out of your code and it resolves: CheckCircle2 finds circle-check.
...
```
678 icons on a 24×24 grid, in three styles and two corner treatments: stroke, duotone and fill, cut rounded or sharp. MIT licensed, free for commercial work, no attribution required.

568 component sets, each with three variant properties, Container, Style and Corners, so you switch between regular, square and circle, between the three styles, and between rounded and sharp, without swapping components. 56 icons carry a square- form and 54 a circle-.

Which styles an icon has is measured rather than chosen: duotone and fill need a fillable region, so an open glyph like bar-chart is stroke-only, and square-bar-chart has all three. That is why the three counts differ: stroke 678, duotone 556, fill 506.
