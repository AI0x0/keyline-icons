...
* Duotone: 585 icons, a 40% plate under the line
* Fill: 533 icons, solid where the glyph has a region to fill
...
Which styles an icon has is measured rather than chosen: duotone and fill need a fillable region, so an open glyph like bar-chart is stroke-only, and square-bar-chart has all three. That is why the three counts differ: stroke 721, duotone 585, fill 533.
